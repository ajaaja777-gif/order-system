...
export default function Calculator() {
  ...
  const [selectedBoard, setSelectedBoard] = useState('MDF_18t');
  ...
  const [compBoard, setCompBoard] = useState('MDF 18t');
  ...
  const boardUnitCost = costDb[selectedBoard] || 0;
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px', fontFamily: 'sans-serif' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', paddingBottom: '12px', borderBottom: '2px solid #333' }}>
        <h1 style={{ margin: 0, fontSize: '24px', fontWeight: 'bold' }}>🧮 원가 및 경쟁사 단가 관리기</h1>
      ...
      <div style={{ background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px', padding: '16px', marginBottom: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <h2 style={{ fontSize: '16px', fontWeight: 'bold', margin: 0, color: '#1e293b' }}>💾 저장된 자재/가공 기본 단가</h2>
          {!isEditing ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '10px', fontSize: '13px' }}>
          {Object.keys(costDb).map((key) => (
            <div key={key} style={{ background: '#fff', padding: '8px 12px', borderRadius: '6px', border: '1px solid #e2e8f0' }}>
              <div style={{ color: '#64748b', fontSize: '11px' }}>{key}</div>
              {isEditing ? (
                <input
                  type="number"
                  value={costDb[key]}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', marginBottom: '32px' }}>
        <div style={{ background: '#fff', padding: '20px', borderRadius: '8px', border: '1px solid #d1d5db' }}>
          <h2 style={{ fontSize: '18px', fontWeight: 'bold', marginTop: 0, marginBottom: '16px', color: '#2563eb' }}>📦 품목 견적 산출</h2>
          <div style={{ display: 'grid', gap: '12px' }}>
            <div>
              <label style={{ fontSize: '13px', fontWeight: 'bold' }}>보드 선택</label>
              <select value={selectedBoard} onChange={(e) => setSelectedBoard(e.target.value)} style={{ width: '100%', padding: '8px', marginTop: '4px' }}>
                <option value="MDF_18t">MDF 18t ({costDb['MDF_18t']?.toLocaleString()}원)</option>
                <option value="MDF_15t">MDF 15t ({costDb['MDF_15t']?.toLocaleString()}원)</option>
                <option value="PB_18t">PB 18t ({costDb['PB_18t']?.toLocaleString()}원)</option>
              </select>
            </div>
            <div>
        ...
        <div style={{ background: '#f0fdf4', padding: '20px', borderRadius: '8px', border: '1px solid #bbf7d0' }}>
          ...
          <div style={{ background: '#fff', padding: '12px', borderRadius: '6px', marginBottom: '8px', border: '1px solid #dcfce7' }}>
            <div style={{ fontSize: '12px', color: '#65a30d' }}>장당 원가 (로스 포함)</div>
      ...
      <div style={{ background: '#fff', border: '1px solid #cbd5e1', borderRadius: '8px', padding: '20px' }}>
        ...
        <form onSubmit={handleAddCompetitorPrice} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '10px', marginBottom: '20px', background: '#f8fafc', padding: '12px', borderRadius: '6px' }}>
          <input type="text" placeholder="경쟁업체명 (예: A상사)" value={compName} onChange={(e) => setCompName(e.target.value)} style={{ padding: '8px', borderRadius: '4px', border: '1px solid #ccc' }} required />
          <input type="text" placeholder="보드 (예: MDF 18t)" value={compBoard} onChange={(e) => setCompBoard(e.target.value)} style={{ padding: '8px', borderRadius: '4px', border: '1px solid #ccc' }} />
          ...
          <input type="number" placeholder="판매 단가(원/장)" value={compPrice} onChange={(e) => setCompPrice(e.target.value === '' ? '' : Number(e.target.value))} style={{ padding: '8px', borderRadius: '4px', border: '1px solid #ccc' }} required />
          <input type="text" placeholder="특이사항 메모" value={compMemo} onChange={(e) => setCompMemo(e.target.value)} style={{ padding: '8px', borderRadius: '4px', border: '1px solid #ccc' }} />
          <button type="submit" style={{ background: '#0284c7', color: '#fff', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>+ 기록 저장</button>
        </form>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          ...
          <tbody>
            {competitorList.length === 0 ? (
              <tr>
                <td colSpan={6} style={{ padding: '16px', textAlign: 'center', color: '#94a3b8' }}>기록된 경쟁사 단가 정보가 없습니다.</td>
              </tr>
            ) : (
